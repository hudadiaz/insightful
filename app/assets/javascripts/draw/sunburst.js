    ...
    var f = function(parent_node, current_node, i) {
      ...
      if (existing != undefined) {
        ...
        if (measure == 'false')
          ...
          existing.size += JSON.parse(item[measure]);
      ...
      if (current_node.children.length < 1){
        ...
        if (measure == 'false')
          ...
          current_node.size += JSON.parse(current_node.size)+JSON.parse(item[measure]);
      }
...
function drawZoomable(root) {
  var width = $(".container").width() - 30,
      height = $(".container").height() - $("#sequence").height() - $(".title").height() - $(".caption").height() - 60,
